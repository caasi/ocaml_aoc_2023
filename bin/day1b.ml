...
let rec find_first f list =
  ...
  | [] ->
      []
  | x :: xs ->
      if is_digit x then [x] else find_first f xs

let find_digits line =
  List.append
    (find_first transform_digit line)
    (find_first transform_digit_backward (List.rev line))

let parse_line line =
  Aoc.explode_string line |> find_digits |> Aoc.implode_string |> int_of_string

let main () =
  let%lwt score =
    Aoc.lwt_stdin_map parse_line |> Lwt_stream.fold ( + ) |> ( |> ) 0
  in
  Lwt_io.printf "%d\n" score
